...
class AuthPopUp(context: Context, var root: ConstraintLayout, var hideKeyboard: () -> Unit) : PopUp(context, root) {
    ...
    fun createSignIn(calback: (email: String, password: String) -> Unit, forgotPassword: (String, () -> Unit) -> Unit) {
        title.text = StringMaster.myStringMaster!!.login
        button.text = StringMaster.myStringMaster!!.next

        viewsToRemove.clear()
        for (i in 0..7) {
            viewsToRemove.add(View.generateViewId())
        }
        createInput(viewsToRemove[0], viewsToRemove[1], dp(171), InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS, StringMaster.myStringMaster!!.email, popUp)
        createInput(viewsToRemove[2], viewsToRemove[3], dp(242), InputType.TYPE_TEXT_VARIATION_PASSWORD, StringMaster.myStringMaster!!.password, popUp)
        ...
        forgotBtn.text = StringMaster.myStringMaster!!.forgot
    ...
    fun createSignUp(calback: (email: String, password: String, name: String) -> Unit) {
        title.text = StringMaster.myStringMaster!!.sign_up_name
        button.text = StringMaster.myStringMaster!!.sign_up_next

        viewsToRemove.clear()
        for (i in 0..6) {
            viewsToRemove.add(View.generateViewId())
        }
        createInput(viewsToRemove[0], viewsToRemove[1], dp(171), InputType.TYPE_CLASS_TEXT, StringMaster.myStringMaster!!.name, popUp)
        ...
        val emailLeft = InputLeftPopUp(
                ...
                StringMaster.myStringMaster!!.email,
                StringMaster.myStringMaster!!.sign_up_email,
                StringMaster.myStringMaster!!.sign_up_pre_final,
                viewsToRemove[2],
                viewsToRemove[3],
                hideKeyboard
        )
        ...
        val passLeft = InputLeftPopUp(
                ...
                StringMaster.myStringMaster!!.password,
                StringMaster.myStringMaster!!.sign_up_password,
                StringMaster.myStringMaster!!.sign_up_final,
                viewsToRemove[4],
                viewsToRemove[5],
                hideKeyboard)
        ...
        hintText.text = context.getString(R.string.password_hint)
        ...
        emailLeft.next = {
            if (emailField.text.toString().isNotEmpty()) {
                ...
                hideKeyboard()
                fireAlert(context, StringMaster.myStringMaster!!.fields_empty_title, StringMaster.myStringMaster!!.email_field_empty_msg)
            }
        }
        ...
        passLeft.next = {
            if (passwordField.text.length >= 6){
                ...
                if (passwordField.text.toString().isNotEmpty()) {
                    ...
                    fireAlert(context, StringMaster.myStringMaster!!.fields_empty_title, StringMaster.myStringMaster!!.fields_empty_msg)
                }
            } else {
                fireAlert(context,StringMaster.myStringMaster!!.fields_empty_title,StringMaster.myStringMaster!!.fields_empty_msg)
            }
